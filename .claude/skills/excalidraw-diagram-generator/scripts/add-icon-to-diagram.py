...
Options:
    ...
    --use-edit-suffix      Edit via .excalidraw.edit to avoid editor
                           overwrite issues (enabled by default;
...
def transform_icon_elements(
    ...
    for element in elements:
        ...
        if "groupIds" in new_element:
            new_element["groupIds"] = [group_id_mapping[gid] for gid in new_element["groupIds"]]
...
def add_icon_to_diagram(
    ...
    if label and transformed_elements:
        ...
        _ = max_y - min_y  # icon_height (unused)
    ...
    print(
        ...
        f"(total: {original_count} -> {len(diagram['elements'])})",
    )
...
def main():
    ...
    if len(sys.argv) < 5:
        ...
        print(
            ...
            " editor overwrite issues"
            " (enabled by default; use --no-use-edit-suffix to disable)",
        )
        print("\nExamples:")
